...
def create_booking(asset_id: int, employee_id: int, start_time, end_time):
    ...
    conflict = booking_repo.find_overlap(asset_id, start_time, end_time)
    if conflict:
        raise AppException(409, "This slot overlaps an existing booking", "BOOKING_OVERLAP")

    with get_cursor() as cursor:
        booking = booking_repo.create_booking(asset_id, employee_id, start_time, end_time, cursor=cursor)
    ...
    logger.info("booking created asset_id=%s employee_id=%s", asset_id, employee_id)
    return booking


def list_bookings(employee_id=None, asset_id=None):
    if asset_id is not None:
        return booking_repo.list_bookings_for_resource(asset_id)
    if employee_id:
